...
An accessible HTML5 re‑build of the legacy Flash *Gas Retention Simulator*
(Nebraska Astronomy Applet Project). Behaviour is ported from the decompiled
...
All mathematics and symbols (chemical formulas such as H₂O, axis numbers, units,
percentages, the speed values) are authored as **MathJax/LaTeX** and typeset by
the foundation's `kl-unl.js`. MathJax is part of the shared KL‑UNL foundation and
is expected at `foundation/mathjax/tex-mml-chtml.js`. **That library was not
present in the provided export**, and this project is self‑contained (no CDN), so
you must drop the foundation's local MathJax build into `foundation/mathjax/`.
Until it is present the sim still runs and shows readable plain‑text fallbacks
(e.g. `H₂O`, `300 K`, `1000 m/s`), but the right‑click "Show Math As" menu and
full math a11y require the library. See `CONVERSION_NOTES.md`.

## Files

```
index.html            KL‑UNL scaffold: .app-shell + <kl-unl-masthead> + 4 panels
foundation/           copied from the shared KL‑UNL foundation (see CONVERSION_NOTES
                        for the one corrected file, contents.json)
...
assets/               reused exported assets (none needed — all art is code‑drawn)
